// Blowfish — Bruce Schneier, 1993. Reference constants are public domain.
// Clean-room port based on the well-known Paul Kocher PD reference.
...
// P-array: 18 32-bit values, initial fractional digits of pi.
constexpr uint32_t INITIAL_P[18] = {
...
// S-boxes: 4 x 256 32-bit values, also from pi.
constexpr uint32_t INITIAL_S[4][256] = {
{
...
inline uint32_t feistel(const Context &ctx, uint32_t x) {
    uint32_t h = ctx.S[0][(x >> 24) & 0xFF] + ctx.S[1][(x >> 16) & 0xFF];
    return (h ^ ctx.S[2][(x >> 8) & 0xFF]) + ctx.S[3][x & 0xFF];
}

void encrypt_words(const Context &ctx, uint32_t &xl, uint32_t &xr) {
    uint32_t L = xl;
    uint32_t R = xr;
    for (int i = 0; i < 16; ++i) {
        L ^= ctx.P[i];
...
void key_setup(Context &ctx, const uint8_t *key, size_t key_len) {
    std::memcpy(ctx.P, INITIAL_P, sizeof(INITIAL_P));
    std::memcpy(ctx.S, INITIAL_S, sizeof(INITIAL_S));
